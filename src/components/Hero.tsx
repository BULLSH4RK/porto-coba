...
export function Hero() {
  ...
  const linkStyles: Record<string, string> = {
    amber: "glass-hover glass-amber text-foreground",
    solid: "bg-amber text-[#23212C] hover:bg-amber/85 font-bold",
  }
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center px-6 pt-24 pb-16">
      ...
      >
        <div className="absolute -bottom-24 -right-24 w-64 h-64 rounded-full bg-amber/10 blur-3xl pointer-events-none" />

        <div className="flex flex-col md:flex-row items-center gap-10">
          <motion.div variants={fadeUp} className="relative shrink-0">
            <div className="absolute -inset-2 rounded-[32px] bg-amber/20 blur-2xl" />
            <div className="relative w-32 h-32 md:w-36 md:h-36 rounded-3xl rotate-6 bg-gradient-to-tr from-amber/40 to-white/10 p-px shadow-2xl">
          ...
          <motion.div variants={fadeUp} className="flex-1 text-center md:text-left">
            ...
            <h1 className="font-display text-4xl md:text-6xl font-bold text-foreground leading-tight mb-2">
              ...
              <span className="block bg-clip-text text-transparent bg-gradient-to-r from-amber via-amber/80 to-white text-glow">
        ...
        >
          {profile.stats.map((stat) => (
            <motion.div key={stat.label} variants={fadeUp} className="text-center md:text-left">
              <p className="font-display text-lg md:text-2xl font-bold text-amber">{stat.value}</p>
